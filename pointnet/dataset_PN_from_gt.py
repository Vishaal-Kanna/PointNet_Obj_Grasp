...
class ShapeNetDataset1(data.Dataset):
    def __init__(self,
                 root,
                 npoints=2500,
...
class ShapeNetDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.test==False:
            ...
            gt = np.loadtxt(fn[2]).astype(np.int64)
        # print(fn[0])
        # print(point_set.shape)
        point_set_before_transform = point_set.copy()
        ...
        # resample
        point_set = point_set[choice, :]
        point_set_before_transform = point_set_before_transform[choice, :]
        # point_set = point_set[:2500, :]

        # import pptk

        # v = pptk.viewer(point_set)
        ...
        point_set = torch.from_numpy(point_set)
        # print(point_set.shape)
        point_set_before_transform = torch.from_numpy(point_set_before_transform)
        ...
        if self.test == True:
            return point_set, choice, point_set_before_transform
        else:
            return point_set, seg, gt, point_set_before_transform
